...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    //https://my-project-portfolio-backend-production.up.railway.app/contact
  try{
      const res = await fetch("http://localhost:8080/contact" , { method: "POST", headers:{ "Content-Type" : "application/json", }, body: JSON.stringify(formdata)});
      if(res.ok){
        setFormdata({name:"",email:"",message:""});git
        setLoading(false);
        setOpen(true);
      }
    }
